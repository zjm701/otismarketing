window.Modernizr = (function(A, c, g) {
	var I = "2.6.2", w = {}, y = true, L = c.documentElement, a = "modernizr", H = c.createElement(a), E = H.style, K = c.createElement("input"), C = ":)", b = {}.toString, j = " -webkit- -moz- -o- -ms- ".split(" "), h = "Webkit Moz O ms", F = h.split(" "), J = h.toLowerCase().split(" "), G = {
		svg : "http://www.w3.org/2000/svg"
	}, l = {}, p = {}, f = {}, e = [], k = e.slice, s, n = function(U, W, O, V) {
		var N, T, Q, R, M = c.createElement("div"), S = c.body, P = S || c.createElement("body");
		if (parseInt(O, 10)) {
			while (O--) {
		...
		function M(O, Q) {
			...
			if (!P) {
				...
				if (Q.setAttribute && Q.removeAttribute) {
					Q.setAttribute(O, "");
					P = r(Q[O], "function");
					if (!r(Q[O], "undefined")) {
						Q[O] = g
	...
	if (!r(i, "undefined") && !r(i.call, "undefined")) {
		...
		x = function(M, N) {
			return (( N in M) && r(M.constructor.prototype[N], "undefined"))
		}
	}
	if (!Function.prototype.bind) {
		Function.prototype.bind = function d(O) {
			var P = this;
			if ( typeof P != "function") {
				throw new TypeError()
			}
			var M = k.call(arguments, 1), N = function() {
				if (this instanceof N) {
					...
					return P.apply(O, M.concat(k.call(arguments)))
				}
			};
			return N
		}
	...
	function m(Q, M, P) {
		var N = Q.charAt(0).toUpperCase() + Q.slice(1), O = (Q + " " + F.join(N + " ") + N).split(" ");
		if (r(M, "string") || r(M, "undefined")) {
			return z(O, M)
		} else {
			O = (Q + " " + (J).join(N + " ") + N).split(" ");
			return q(O, M, P)
		}
	}
	l.flexbox = function() {
	...
	l.touch = function() {
		...
		if (("ontouchstart" in A) || A.DocumentTouch && c instanceof DocumentTouch) {
			...
			n(["@media (", j.join("touch-enabled),("), a, ")", "{#modernizr{top:9px;position:absolute}}"].join(""), function(N) {
				M = N.offsetTop === 9
			})
		}
		return M
	...
	l.cssgradients = function() {
		var O = "background-image:", N = "gradient(linear,left top,right bottom,from(#9f9),to(white));", M = "linear-gradient(left top,#9f9, white);";
	...
	l.csstransforms3d = function() {
		var M = !!m("perspective");
		if (M && "webkitPerspective" in L.style) {
			n("@media (transform-3d),(-webkit-transform-3d){#modernizr{left:9px;position:absolute;height:3px;}}", function(N, O) {
	...
	l.fontface = function() {
		var M;
		n('@font-face {font-family:"font";src:url("https://")}', function(Q, R) {
			var P = c.getElementById("smodernizr"), N = P.sheet || P.styleSheet, O = N ? (N.cssRules && N.cssRules[0] ? N.cssRules[0].cssText : N.cssText || "") : "";
	...
	l.generatedcontent = function() {
		var M;
		n(["#", a, "{font:0/0 a}#", a, ':after{content:"', C, '";visibility:hidden;font:3px/1 a}'].join(""), function(N) {
			M = N.offsetHeight >= 3
		});
		return M
	};
	l.video = function() {
		var N = c.createElement("video"), M = false;
		try {
			if ( M = !!N.canPlayType) {
				M = new Boolean(M);
	...
	l.svg = function() {
		return !!c.createElementNS && !!c.createElementNS(G.svg, "svg").createSVGRect
	};
	...
	l.smil = function() {
		return !!c.createElementNS && /SVGAnimate/.test(b.call(c.createElementNS(G.svg, "animate")))
	};
	l.svgclippaths = function() {
		return !!c.createElementNS && /SVGClipPath/.test(b.call(c.createElementNS(G.svg, "clipPath")))
	};
	function B() {
		w.input = (function(O) {
		...
		w.inputtypes = (function(P) {
			for (var O = 0, N, R, Q, M = P.length; O < M; O++) {
				K.setAttribute("type", R = P[O]);
				N = K.type !== "text";
				if (N) {
					K.value = C;
					K.style.cssText = "position:absolute;visibility:hidden;";
					if (/^range$/.test(R) && K.style.WebkitAppearance !== g) {
						L.appendChild(K);
						Q = c.defaultView;
						N = Q.getComputedStyle && Q.getComputedStyle(K, null).WebkitAppearance !== "textfield" && (K.offsetHeight !== 0);
						L.removeChild(K)
					} else {
						if (/^(search|tel)$/.test(R)) {
						} else {
							if (/^(url|email)$/.test(R)) {
								N = K.checkValidity && K.checkValidity() === false
							} else {
								N = K.value != C
							}
						}
	...
	( function(V, X) {
			...
			( function() {
					try {
						var ae = X.createElement("a");
						ae.innerHTML = "<xyz></xyz>";
						ab = ("hidden" in ae);
						Q = ae.childNodes.length == 1 || ( function() {
								(X.createElement)("a");
								var ag = X.createDocumentFragment();
			...
			function R(ae, ag) {
				var ah = ae.createElement("p"), af = ae.getElementsByTagName("head")[0] || ae.documentElement;
				ah.innerHTML = "x<style>" + ag + "</style>";
				return af.insertBefore(ah.lastChild, af.firstChild)
			}
			...
			function Y(ah, ae, ag) {
				...
				var af;
				if (ag.cache[ah]) {
					af = ag.cache[ah].cloneNode()
				} else {
					if (N.test(ah)) {
						af = (ag.cache[ah] = ag.createElem(ah)).cloneNode()
					} else {
						af = ag.createElem(ah)
					}
				}
				return af.canHaveChildren && !S.test(ah) ? ag.frag.appendChild(af) : af
			}
			...
			function ac(ag, ai) {
				...
				if (Q) {
					return ag.createDocumentFragment()
				}
				ai = ai || aa(ag);
				var aj = ai.frag.cloneNode(), ah = 0, af = W(), ae = af.length;
				for (; ah < ae; ah++) {
					aj.createElement(af[ah])
				}
				return aj
			}
			...
			function ad(ae, af) {
				if (!af.cache) {
					af.cache = {};
					af.createElem = ae.createElement;
					af.createFrag = ae.createDocumentFragment;
					af.frag = af.createFrag()
				}
				ae.createElement = function(ag) {
					if (!U.shivMethods) {
						return af.createElem(ag)
					}
					return Y(ag, ae, af)
				};
				ae.createDocumentFragment = Function("h,f","return function(){var n=f.cloneNode(),c=n.createElement;h.shivMethods&&("+W().join().replace(/\w+/g,function(ag){af.createElem(ag);af.frag.createElement(ag);return'c("'+ag+'")'})+");return n}")(U, af.frag)
			}
			...
			function O(ae) {
				...
				var af = aa(ae);
				if (U.shivCSS && !ab && !af.hasCSS) {
					af.hasCSS = !!R(ae, "article,aside,figcaption,figure,footer,header,hgroup,nav,section{display:block}mark{background:#FF0;color:#000}")
				}
				if (!Q) {
					ad(ae, af)
				}
			...
			var U = {
				...
				shivCSS : (P.shivCSS !== false),
				supportsUnknownElements : Q,
				shivMethods : (P.shivMethods !== false),
				type : "default",
				shivDocument : O,
				createElement : Y,
				createDocumentFragment : ac
			};
			V.html5 = U;
			O(X)
		}(this, c));
	...
	w._cssomPrefixes = F;
	w.hasEvent = o;
	w.testProp = function(M) {
		return z([M])
	};
	w.testAllProps = m;
	w.testStyles = n;
	w.prefixed = function(O, N, M) {
		if (!N) {
			return m(O, "pfx")
		} else {
			return m(O, N, M)
...
(function(ad, ac, ab) {
	...
	var S = ac.documentElement, R = ad.setTimeout, Q = ac.getElementsByTagName("script")[0], P = {}.toString, O = [], M = 0, K = "MozAppearance" in S.style, J = K && !!ac.createRange().compareNode, I = J ? S : Q.parentNode, S = ad.opera && "[object Opera]" == P.call(ad.opera), S = !!ac.attachEvent && !S, H = K ? "object" : S ? "script" : "img", G = S ? "script" : H, F = Array.isArray ||
	function(b) {
